...
function MapComponent() {
  // HTML div reference where Leaflet will mount
  const mapContainerRef = useRef(null);

  // Store the Leaflet map and routing control in refs so we only create them once
  ...
  // Example list of campus POIs (add your real data and coords here)
//   const locations = [
//     { name: 'Library', coords: [40.7128, -74.0060] },
//     { name: 'Cafeteria', coords: [40.7138, -74.0055] },
...
const locations = [
  ...
  useEffect(() => {
    if (!mapRef.current) {
      ...
      mapRef.current = L.map(mapContainerRef.current, {
        center: [26.51138333, 80.23493056], // Example center (NYC). Replace with your campus coords
        zoom: 17,
      });

      // 2) Add an OSM tile layer
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        attribution: '© OpenStreetMap contributors',
      }).addTo(mapRef.current);

      // 3) Add markers for each POI
      ...
      // 4) Initialize the routing control (with no waypoints yet)
      routingControlRef.current = L.Routing.control({
        waypoints: [],
        routeWhileDragging: false,
        showAlternatives: false,
        // By default, it uses OSRM driving. 
        // For a campus, you might want: profile: 'foot' or a custom server.
        // serviceUrl: 'https://router.project-osrm.org/route/v1/driving',
      }).addTo(mapRef.current);
    }
  }, [locations]);

  // Handler to update the route based on selected dropdown values
  const handleRoute = () => {
  ...
  return (
    ...
    <div style={{ width: '100%', height: '100%' }}>
      {/* Control panel for selecting start/end + route button */}
...
// Some quick inline styling for the control panel
const styles = {
    controlsWrapper: {
        display: 'flex',
        flexWrap: 'wrap',
